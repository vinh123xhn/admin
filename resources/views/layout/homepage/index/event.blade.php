...
<div id="content-news" class="content-news">
    @foreach($posts as $k => $item)
        ...
        <ul class="pagination pagination-event justify-content-center">
            @if($posts->lastPage() > 1)
                <li class="page-item">
            ...
            @for($i = 1; $i <= $posts->lastPage(); $i++)
                <li class="page-item">
                    <a role="button" class="page-link {{$i == $posts->currentPage() ? 'active' : ''}}" onclick="pagination({{$i}})">
                        {{$i}}
                    </a>
                </li>
            @endfor
